...
adminRouter.get("/", authorize, onlyForAdmin, async (req, res, next) => {
  try {
    ...
    for (queryParam in req.query) {
      //for each value in query string, I'll filter
    ...
    if (sort !== undefined) query += `ORDER BY ${sort} ${order}`; //adding the sorting
...
adminRouter.put("/me", authorize, async (req, res, next) => {
  try {
    ...
    if (result.rowCount === 0)
      //if no element match the specified id => 404
      return res.status(404).send("Not Found");

    const updateUserInfo = await db.query(
      'UPDATE "users" SET email = $1 WHERE email = $2',
      [req.body.email, req.user.email]
    );

    res.send(result.rows[0]); //else, return the updated versions
  } catch (error) {
    next(error);
  }
});
...
adminRouter.delete("/me", authorize, async (req, res, next) => {
  try {
    const response = await db.query(`DELETE FROM "admin" WHERE email = $1`, [
      req.user.email,
    ]);
